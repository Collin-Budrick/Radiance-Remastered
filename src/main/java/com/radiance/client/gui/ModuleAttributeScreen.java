...
public class ModuleAttributeScreen extends Screen {
    ...
    protected void init() {
        ...
        for (AttributeConfig cfg : list) {
            ...
            List<AbstractWidget> ws = AttributeWidgetUtil.buildWidgets(module, cfg, this.font, WIDGET_WIDTH,
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.text(this.font, module.translateText(module.name), 10, HEADER_HEIGHT + 8, 0xFFEAEAEA);

        if (rows.isEmpty()) {
            context.text(this.font, MODULE_ATTRIBUTE_SCREEN_NO_ATTRIBUTES, 10, 60, 0xFFB0B0B0);
            return;
        }
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            if (visible) {
                context.text(this.font, module.translateText(row.cfg.name), ROW_LEFT, y + 6,
                    0xFFD0D0D0);
            }
